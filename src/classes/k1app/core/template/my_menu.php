...
class my_menu extends menu {
    public function __construct($menu_title = null) {
        if (isset(app_session::get_user_data()['user_login'])) {
            ...
            $person_profile_url = K1APP_URL . 'app/public/user/perfil/' . $user_login . '/?auth-code=' . $user_auth_code;
        ...
        if (app_session::check_user_level(['god'])) {
            ...
            $submenu_config->add_subitem(
                'Config a table',
                K1APP_BASE_URL . 'core/admin/select_table/',
                'nav-config-table'
            );
            $submenu_config->add_subitem(
                'Export configuration',
                K1APP_BASE_URL . 'core/admin/export_db_configuration/',
                'nav-export-configuration'
            );
            $submenu_config->q('#a-nav-export-configuration')->set_attrib('target', '_export');
            $submenu_config->add_subitem(
                'Import configuration',
                K1APP_BASE_URL . 'core/admin/import_db_configuration/',
                'nav-export-configuration'
            );
        }
    }
