        ...
        extends menu
{
    ...
    public function __construct($menu_title = null)
    {
        ...
        $this->add_item(
                'Inicio', K1APP_URL, 'bi bi-file-earmark-bar-graph', 'nav-index'
        );
        if (!app_session::is_logged())
        {
            $this->add_item(
                    'Iniciar sesión', K1APP_URL . 'auth/login/', 'bi bi-person-badge-fill', 'nav-login'
            );
            $this->add_item(
        ...
        {
            $this->add_item(
                    'Mi perfil', $person_profile_url, 'bi bi-person-badge-fill', 'nav-my-profile'
            );
            $this->add_item(
                    'Cerrar sesión', K1APP_URL . 'auth/logout/', 'bi bi-door-open', 'nav-login'
            );
        } else
        {
            if (app_session::is_logged())
            {
                $submenu_profile = $this->add_item(
                                'Mi perfil', '#', 'bi bi-person-badge-fill', 'nav-user-profile'
                        )->nav_is_sub();
                $submenu_profile->add_subitem(
                        'Ver perfíl', $user_profile_url, 'nav-my-profile'
                );
                $submenu_profile->add_subitem(
                        'Cerrar sesión', K1APP_URL . 'auth/logout/'
                );
            } else
            {
                $this->add_item(
        ...
        $submenu_layouts->add_subitem(
                'Blank page', K1APP_BASE_URL . 'app/examples/layout/blank/', 'nav-blank-page'
        );
        $submenu_layouts->add_subitem(
        ...
        $submenu_layouts->add_subitem(
                'Sidebar and page No Card', K1APP_BASE_URL . 'app/examples/layout/sidebar_page_no_card/',
                'nav-sidebar-page-no-card'
        );

        /**
         * MENU SECTION - EXAMPLES
         */
        $submenu_exmaples = $this->add_item(
                        'Examples', '#', 'bi bi-file-earmark-medical-fill'
                )->nav_is_sub();
        $submenu_exmaples->add_subitem(
                'Profile', K1APP_BASE_URL . 'app/examples/profile/', 'nav-profile-page'
        );

        /**
         * MENU SECTION - CRUD
         */
        $submenu_crud = $this->add_item(
                        'CRUD', '#', 'bi bi bi-table'
                )->nav_is_sub();
        $submenu_crud->add_subitem(
                'Simple Table', K1APP_BASE_URL . 'app/examples/crud/table_simple/', 'nav-simple-crud'
        );
        $submenu_crud->add_subitem(
        ...
        if (app_session::check_user_level(['god']))
        {
            ...
            $submenu_config = $this->add_item(
                            'DB Configurator', '#', 'bi bi-database', 'nav-db-configurator'
                    )->nav_is_sub();
